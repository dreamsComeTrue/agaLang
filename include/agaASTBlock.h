...
namespace aga
{
    ...
    {
      public:
        agaASTBlock (agaASTBlock *parentBlock) : agaASTNode (BlockNode), m_Parent (parentBlock) {}

        void AddStatement (agaASTNode *node) { m_Statements.push_back (node); }

        const std::vector<agaASTNode *> &GetStatements () { return m_Statements; }

        const agaASTNode *GetParent () { return m_Parent; }
